...
public class IndexController {//inedx.mustache를 View Resolver가 처리
    ...
    @GetMapping("/")
    public String index(Model model, @LoginUser SessionUser user){//Model - 서버 템플릿 엔진에서 사용할 수 있는 객체를 저장할 수 있다. postsService.findAllDesc()로 가져온 결과를 posts로 index.mustache에 전달
